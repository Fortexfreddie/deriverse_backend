...
async function fetchMyTrades() {
  ...
  try {
    // Try to get signatures - the API might be different in @solana/kit
    // First, let's try querying by program ID to get all program transactions
    ...
    for (let i = 0; i < Math.min(successfulSignatures.length, 10); i++) {
      ...
      try {
        ...
        for (const log of logMessages) {
          // Look for "Program data: <base64>" pattern
        ...
        if (programDataLogs.length > 0) {
          ...
          // First, try decoding all logs together (this is often more reliable)
          try {
            ...
            // Fallback: Try decoding individual Base64 entries
            ...
            for (let idx = 0; idx < Math.min(programDataLogs.length, 3); idx++) {
              ...
              try {
                // Try different log formats that logsDecode might expect
                const formats = [
          ...
          // If no "Program data:" found, try decoding all logs anyway
        ...
        // 6. Parse decoded events for trade information
        for (const event of decodedEvents) {
          // Check if this is a trade event based on tag or event structure
          // Tag 11 and 19 appear to be trade/fill events based on the decoded data
          ...
          if (isTradeEvent) {
            // Map side: 0 = BUY, 1 = SELL (based on decoded event structure)
            const side = event.side === 0 ? "BUY" : 
                        event.side === 1 ? "SELL" : "UNKNOWN";
            
            // Extract trade data from decoded event
            // Price is typically in event.price (already decoded as float)
            const price = event.price || null;
            
            // Size/quantity can be in qty (spot) or perps (perpetual)
            // These are often in raw units (e.g., 1000000000 = 1.0 with 9 decimals)
            const rawSize = event.qty || event.perps || null;
            const size = rawSize ? Number(rawSize) / 1e9 : null; // Convert from raw units if needed
            
            // Fee/rebates: rebates might be negative fees or fee rebates
            const rebates = event.rebates || null;
            const fee = rebates ? -Number(rebates) / 1e9 : null; // Convert and negate if rebates represent fees
        ...
        // 7. Fallback: Parse raw log messages for trade events (if decoding didn't find anything)
        if (decodedEvents.length === 0) {
          for (const log of logMessages) {
            const lowerLog = log.toLowerCase();
            
            // Check for various trade event patterns in raw logs
            if (lowerLog.includes("taker buy") || lowerLog.includes("taker sell") || 
              ...
              // Try to extract price, size, fee from log
        ...
        // Also print all logs for debugging
